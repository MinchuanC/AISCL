...
print("SNR | BER_SCL | BER_AISCL | FLOPS_SCL | FLOPS_AISCL | TIME_SCL(ms) | TIME_AISCL(ms)")
import time
for snr in snr_range:
    ber_scl, ber_aiscl = 0, 0
    flops_scl_acc, flops_aiscl_acc = [], []
    ...
    for _ in range(messages):
        ...
        flops_scl_acc.append(scl_codec.decoder.op_counts.get('flops', 0))
        ...
        flops_aiscl_acc.append(aiscl_codec.decoder.op_counts.get('flops', 0))
        ber_aiscl += compute_fails(msg, dec_aiscl)[0]
    
    ber_scl_val = ber_scl / (messages * K)
    ber_aiscl_val = ber_aiscl / (messages * K)
    avg_flops_scl = np.mean(flops_scl_acc) if flops_scl_acc else 0
    avg_flops_aiscl = np.mean(flops_aiscl_acc) if flops_aiscl_acc else 0
    avg_time_scl = np.mean(times_scl)
    avg_time_aiscl = np.mean(times_aiscl)
    
    print(f"{snr:4.1f} | {ber_scl_val:.6e} | {ber_aiscl_val:.6e} | {avg_flops_scl:9.0f} | {avg_flops_aiscl:10.0f} | {avg_time_scl:12.3f} | {avg_time_aiscl:12.3f}")
